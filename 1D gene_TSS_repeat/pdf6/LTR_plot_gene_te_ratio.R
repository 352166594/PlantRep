...
split1 <- p + coord_cartesian(ylim = c(0.02, 0.11)) +
  theme(legend.position='none',axis.line=element_line(size=1,colour="black"))
split2 <- p + coord_cartesian(ylim = c(0, 0.004)) +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), legend.position = 'none',axis.line.y=element_line(size=1,colour="black"))

#使用 grid 包组合分割结果
#png('3box_plot.split.png', width = 1600, height = 1600, res = 300, units = 'px')
#pdf('3box_plot.split.pdf', width = 1600, height = 1600)
pdf('LTR_plot.split.pdf')
grid.newpage()
plot_site1 <- viewport(x = 0, y = 0, width = 1, height = 0.7, just = c('left', 'bottom'))
plot_site2 <- viewport(x = 0, y = 0.7, width = 1, height = 0.3, just = c('left', 'bottom'))
