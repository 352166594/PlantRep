...
f2=open("copia_percentage_species_count_insert_time_format.xls","w")
f3=open("copia_percentage_kindom_count_insert_time_format.xls","w")
dict_kindom={}
list_kindom=[]
numb_kindom={}
...
#		os.chdir("/vol3/agis/huangsanwen_group/luoxizhi/work/525_harvast/species_percentage_insert_time")
		os.chdir("/vol3/agis/huangsanwen_group/luoxizhi/work/525_harvast/copia_gypsy/copia_species_percentage_insert_time")
...
#		print type
	for tim in range(len(time_list)):
		if "Million" in time_list[tim]:
			...
			#f3.write("{0}\t{1}\t{2}\t{3}\t{4}\n".format(type,time_list[tim].split("-")[0],"Million",dict_kindom[type][time_list[tim]] ,Figure_order))
			#f3.write("{0}\t{1}\t{2}\t{3}\t{4}\n".format("_".join(type.split("_")[1:]),time_list[tim].split("-")[0],"Million",dict_kindom[type][time_list[tim]]/numb_kindom[type][time_list[tim]] ,Figure_order))
			f3.write("{0}\t{1}\t{2}\t{3}\t{4}\n".format(type,time_list[tim].split("-")[0],"Million",dict_kindom[type][time_list[tim]]/numb_kindom[type][time_list[tim]] ,Figure_order))
		elif "Hundred_thousand" in time_list[tim]:
			Interval=int(time_list[tim].split("-")[0])
			
			
			Figure_order=tim+1-49
#			f3.write("{0}\t{1}\t{2}\t{3}\t{4}\n".format(type,time_list[tim].split("-")[0],"Hundred_thousand",dict_kindom[type][time_list[tim]]/numb_kindom[type][time_list[tim]] ,Figure_order))
			f3.write("{0}\t{1}\t{2}\t{3}\t{4}\n".format(type,time_list[tim].split("-")[0],"Hundred_thousand",dict_kindom[type][time_list[tim]]/numb_kindom[type][time_list[tim]] ,Figure_order))
