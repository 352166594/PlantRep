...
f2=open("percentage_species_count_insert_time_format.xls","w")
f3=open("percentage_kimdom_count_insert_time_format.xls","w")
dict_kindom={}
list_kindom=[]
numb_kindom={}
...
for lin1 in f1:
	...
	if "Species_type" in lin1:
		...
		fout.close()
		os.chdir("/vol3/agis/huangsanwen_group/luoxizhi/work/525_harvast/")
		for time in range(len(lis1[5:])):
...
for type in list_kindom:
	for tim in range(len(time_list)):
		if "Million" in time_list[tim]:
			...
			f3.write("{0}\t{1}\t{2}\t{3}\t{4}\n".format(type,time_list[tim].split("-")[0],"Hundred_thousand",dict_kindom[type][time_list[tim]] ,tim+1))
